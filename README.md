...
* 모든 선택자 : `* {속성:값;}`
* 수열(nth) 선택자
    * 형제가 2개 이상일 때 원하는 형제를 선택하는 선택자(무조건 형제가 2개 이상인 li, dt-dd가 많이 사용)
...
* `margin` : 바깥쪽(형제 사이) 여백(피그마 기준**간격**)
    ...
    * `margin:10px` : 모든 방향 10통일
    * `margin:10px 20px 30px` : 상/좌우/하
    * `margin:10px 20px 30px 10px` : 상/우/하/좌(시계방향)
    * **위 4개 속성방향은 padding과 동일**
...
## 행 그룹 포함 table 작성 순서
* `table` -> `thead` -> `tr` -> `th`(thead에는 th만 들어감)
* `table` -> `tbody` -> `tr` -> `th`
* `table` -> `tbody` -> `tr` -> `td`
* `table` -> `tfoot` -> `tr` -> `th`
* `table` -> `tfoot` -> `tr` -> `td`
...
* 작성한 html에서만 사용할 수있다는 단점있음.
## `@font-face` 사용법, 특징
* `resey.css` 파일 내 가장 위쪽 라인에 작성
* `@font-face {`
    * 필수 속성
    * `font=famaily:'사용할 글꼴 이름 임의작성'`
    * `src:url(글꼴주소)`
    * 선택 속성
...
* `선택자 {font-family:'웹글꼴로 불러온 글꼴명'}` (예) 포토샵에서글꼴사용
## css 레이아웃 정렬 속성
### display
...
* `li {width:calc((100% - 30px) / 4);}`
    * 4개의 li에 각 10px씩 사이여백을 주기 위해 전체 부모 100%너비 중 10*3 총 10px을 빼고 나머지 값을 4로 나누기
...
* `keyframes + animation`은 페이지 로딩 시 등장 애니메이션, 스크롤 내릴시 나타나는 컨텐츠의 등장 애니메이션 등에 사용함(사용자 hover 동작과 대부분 관계없음)
### 명령 해석
* `@keyframes a {0% {opacity:0;} 100% {opacity:1;}}`
    * 애니메이션이름 a 생성 후 안보이다가 보이는 애니메이션
* `선택자 {animation:a 1s ease 0.5s alternate 3;}`
    * 특정 선택자에 0.5 기다리고 1초동안 애니메이션을 ease 가속도로 정방향 -> 역방향 순서로 3번 재생하기(무한재생은 `infinite`)
## transform
